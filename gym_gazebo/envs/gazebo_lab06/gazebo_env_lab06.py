...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        if len(contours) > 0:
            cnt = contours[0]
            M = cv2.moments(cnt)
            cx = int((M['m10'] + 0.00001)/(M['m00'] + 0.00001))
            cy = int((M['m01'] + 0.00001)/(M['m00'] + 0.00001)) + CUT_SIZE
            state[int((cx) // (len(cv_image[1])/10))] = 1
            self.timeout = 0
            # Mark Image
            gray = cv2.cvtColor(cv_image, cv2.COLOR_RGB2GRAY)
            color = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
            color = cv2.circle(color, (int(cx),int(cy)), RADIUS, (0,0,255), -1)
            color = cv2.putText(color, ''.join([str(c) for c in state]), (10,230), 
